...
static ssize_t data_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
        ...
        u8 code;
        struct MB85RS *drv = dev_get_drvdata(dev);
        unsigned char to_write[count + 3];
        ...
        to_write[0] = OPCODE_WRITE;
        to_write[1] = (u8)(drv->addr >> 8);
        to_write[2] = (u8)(drv->addr & 0xFF);
        memcpy(&to_write[3], buf, count);
        
        status = spi_write(drv->myspi, to_write, count + 3);
...
static int MB85RS_spi_probe(struct spi_device *spi){
    ...
    gpio_request(drv->cspin, "sysfs");
    gpio_direction_output(drv->cspin, 0);
    gpio_export(drv->cspin, false);          // export the cs pin
